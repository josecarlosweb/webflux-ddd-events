...
class CoordinateProcessor(
    ...
    fun receiveCoordinate(coordinate: Coordinate): Mono<NotificationDto> {
        ...
        return routeRepository.getRouteByEquipment_Id(coordinate.equipmentId)
            .flatMap { route -> updateLastCoordinate(coordinate, route) }
            .map { lastCoordinate ->
                NotificationDto(coordinate, lastCoordinate)
            }
    }

    private fun updateLastCoordinate(
        coordinate: Coordinate,
        route: Route
    ) = lastCoordinateRepository.getLastCoordinateByEquipment_Id(coordinate.equipmentId)
        .flatMap { lastCoordinate ->
            val updatedLastCoordinate = lastCoordinate.copy(
                latitude = coordinate.latitude,
                longitude = coordinate.longitude,
                `when` = coordinate.datePing
            )
            lastCoordinateRepository.save(updatedLastCoordinate)
                .thenReturn(lastCoordinate)
        }.switchIfEmpty {
            val newLasCoordinate =
                createLastCoordinate(coordinate.equipmentId, coordinate.latitude, coordinate.longitude, route)
